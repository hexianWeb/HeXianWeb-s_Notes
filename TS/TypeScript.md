...
> TS中 声明和赋值若同时进行 则不需要特地声明变量类型 TS会自动对变量进行类型检测
...
- boolean
  ...
  - ```typescript
    ...
    **注意，使用构造函数Boolean创造的对象不是布尔值，而是一个Boolean对象**
...
- any
  ...
  > .没有强制限定哪种类型，随时切换类型都可以 我们可以对 any 进行任何操作，不需要检查类型 但请不要把您的TypeScript变成AnyScript
...
- unknown
  ...
  - ```typescript
    //unknow  unknow类型比any更加严格当你要使用any 的时候可以尝试使用unknow 他可以定义任何类型的值
    let notSure: unknown = 4;
    
    notSure = 'hello';
    ```
    
    *但是unknow不能作为子类型 any可以作为子类型和父类型，简言就是 unkown类型不能赋值给其他类型*
...
- object
  ...
  - ```typescript
    let obj: object = {};
    //{}可以用来指定对象中包含哪些属性，在属性名后加一个？表示属性为可选属性
...
- 示例（实现）
  ...
  - ```typescript
    ...
    **任意属性 [propName: string]**
    ...
    //在这个例子当中我们看到接口中并没有定义Person.c 但是并没有报错
